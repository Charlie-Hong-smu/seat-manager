...
随机排座默认参考当前座次及最近最多五份含稳定学生 ID 的历史快照，优先满足原有硬约束和软约束，再降低原座重复与邻座重复；锁定座位不计入原座惩罚，只有姓名的旧快照不参与轮换评分。预览与规则共用座位工作面，候选座次留在临时状态，离开或取消时丢弃；预览显示重复情况，老师应用后追加标记为“自动轮换”的普通座位快照；撤销该次排座时一并移除相应快照。开关位于 `seatSettings.rotateWithHistory`，旧设置缺省为开启，仍沿用原座位历史和整柜备份链路。
