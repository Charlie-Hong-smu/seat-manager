...
出勤、学生跟进任务和抽签会话属于当前学期切片数据，随整柜备份和手动云同步一起保存。跟进任务允许 `studentId` 为空字符串，表示不绑定具体学生的班级事项；健康检查与班级动态不得把这种记录判为学生引用损坏。评语与 AI 辅助缓存使用 `workspaceSliceId + studentId` 作为作用域，不能跨班级或学期复用。
