...
export function normalizeFollowupTasks(raw: unknown): FollowupTask[] {
  ...
  return raw.flatMap((value, index): FollowupTask[] => {
    ...
    const source: FollowupTaskSource = ["ai", "score", "attendance", "dormitory"].includes(String(item.source)) ? item.source as FollowupTaskSource : "manual";
    const createdAt = typeof item.createdAt === "string" ? item.createdAt : new Date().toISOString();
    ...
    return [{ id: typeof item.id === "string" ? item.id : `followup-${index}`, studentId: item.studentId, title: item.title.trim(), type: typeof item.type === "string" ? item.type : "常规跟进", description: typeof item.description === "string" ? item.description : "", plannedDate: typeof item.plannedDate === "string" ? item.plannedDate : "", dueDate: typeof item.dueDate === "string" ? item.dueDate : "", status, source, createdAt, updatedAt: typeof item.updatedAt === "string" ? item.updatedAt : createdAt, completedAt: typeof item.completedAt === "string" ? item.completedAt : undefined, lastNotifiedAt: typeof item.lastNotifiedAt === "string" ? item.lastNotifiedAt : undefined, sourceRef: sourceRef && ["dormitory", "attendance", "ai"].includes(String(sourceRef.domain)) && typeof sourceRef.entityId === "string" ? { domain: sourceRef.domain as "dormitory" | "attendance" | "ai", entityId: sourceRef.entityId } : undefined }];
  });
}
