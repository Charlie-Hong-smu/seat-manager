...
function attendanceCardStyle(record?: AttendanceRecord) {
  ...
  if (record?.late && record?.earlyLeave) return { card: "border-amber-200 bg-amber-50/70", badge: "bg-amber-100 text-amber-700", dot: "bg-amber-500 text-white" };
  if (record?.late) return { card: "border-blue-200 bg-blue-50/70", badge: "bg-blue-100 text-blue-700", dot: "bg-blue-500 text-white" };
  if (record?.earlyLeave) return { card: "border-amber-200 bg-amber-50/70", badge: "bg-amber-100 text-amber-700", dot: "bg-amber-500 text-white" };
...
export function AttendanceWorkspace({ students, records, tasks = [], onChange, onRequestTask, onActivity, onOpenTask, initialTarget }: { students: AppStudent[]; records: AttendanceRecord[]; tasks?: FollowupTask[]; onChange: (records: AttendanceRecord[]) => void; onRequestTask: (draft: FollowupTaskDraft) => void; onActivity?: (event: ActivityEvent) => void | (() => void); onOpenTask?: (taskId: string) => void; initialTarget?: TimelineTarget }) {
  ...
  return <div className="h-full overflow-y-auto bg-gray-50 p-4"><div className="mx-auto max-w-6xl space-y-4">
    ...
    <Card title="每日出勤" action={<Button size="sm" variant="ghost" onClick={() => downloadCsv(students, records, date)}><Download className="h-4 w-4"/>导出</Button>}>
      ...
      {viewMode === "quick" ? <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">{rows.map(student => {
        ...
        return <button key={student.id} type="button" data-attendance-student-id={student.id} aria-label={`${student.name}当前${summary}，点击设为${targetLabel}`} onClick={() => markStudent(student)} className={`flex min-h-16 items-center gap-3 rounded-[var(--app-radius-sm)] border px-3 py-2 text-left transition-[border-color,background-color,box-shadow] duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500/25 motion-reduce:transition-none ${style.card} ${recentUpdate?.studentId === student.id || initialTarget?.studentId === student.id ? "ring-2 ring-blue-400 ring-offset-1 shadow-md" : ""}`}>
        ...
        <div className="overflow-hidden rounded-xl border border-gray-100">{rows.map(student => { const record=byStudent.get(student.id); const status=record?.status||"normal"; const editing=editingId===student.id; return <div key={student.id} data-attendance-student-id={student.id} className={`view-switch-enter border-b border-gray-50 px-4 py-3 last:border-0 ${initialTarget?.studentId === student.id ? "entity-focus-highlight" : ""}`}><div className="grid grid-cols-[auto_minmax(7rem,1fr)_auto] items-center gap-3"><input type="checkbox" checked={selected.has(student.id)} onChange={() => toggle(student.id)} className="accent-blue-600" aria-label={`选择 ${student.name}`}/><div><div className="font-bold text-gray-800">{student.name}</div><div className="text-xs text-gray-400">{record?.note || attendanceSummary(record)}</div></div><div className="flex items-center gap-2"><AttendanceStatusControl compact value={status} late={record?.late||false} earlyLeave={record?.earlyLeave||false} onChange={patch => patchStudent(student.id, patch)}/><button onClick={() => setEditingId(editing ? "" : student.id)} className="rounded-lg bg-gray-50 p-2 text-gray-500" aria-label={`编辑 ${student.name} 详情`}><Settings2 className="h-4 w-4"/></button></div></div>{editing && <div className="mt-3 grid gap-2 rounded-xl bg-gray-50 p-3 sm:grid-cols-3"><input value={record?.note||""} onChange={event => patchStudent(student.id,{note:event.target.value})} placeholder="备注" className="h-10 rounded-xl border border-gray-200 bg-white px-3 text-sm outline-none"/><AttendanceDateTimeFields label="请假开始" value={record?.leaveStart||""} onChange={value => patchStudent(student.id,{leaveStart:value})}/><AttendanceDateTimeFields label="请假结束" value={record?.leaveEnd||""} onChange={value => patchStudent(student.id,{leaveEnd:value})}/>{record && <button onClick={() => onRequestTask({studentId:student.id,title:`出勤跟进：${record.status==="leave"?"请假":record.status==="absent"?"缺勤":record.late?"迟到":"早退"}`,type:"出勤关注",description:`${date}${record.note?` · ${record.note}`:""}`,plannedDate:date,dueDate:date,source:"attendance",sourceRef:{domain:"attendance",entityId:record.id,studentId:student.id,date}})} className="sm:col-span-3 flex items-center justify-center gap-1 rounded-lg bg-blue-50 py-2 text-xs font-bold text-blue-700"><ListPlus className="h-3.5 w-3.5"/>创建跟进任务</button>}</div>}</div>})}</div>
      </>}
