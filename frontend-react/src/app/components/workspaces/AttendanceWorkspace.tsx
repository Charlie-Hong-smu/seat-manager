...
function attendanceCardStyle(record?: AttendanceRecord) {
  if (record?.status === "absent") return { card: "border-status-rose-200 bg-status-rose-50/70", badge: "bg-status-rose-100 text-status-rose-700", dot: "bg-status-rose-500 text-text-white" };
  if (record?.status === "leave") return { card: "border-status-warning-200 bg-status-warning-50/70", badge: "bg-status-warning-100 text-status-warning-700", dot: "bg-status-warning-500 text-text-white" };
  if (record?.late && record?.earlyLeave) return { card: "border-status-warning-200 bg-status-warning-50/70", badge: "bg-status-warning-100 text-status-warning-700", dot: "bg-status-warning-500 text-text-white" };
  if (record?.late) return { card: "border-accent-200 bg-accent-50/70", badge: "bg-accent-100 text-accent-700", dot: "bg-accent-500 text-text-white" };
  if (record?.earlyLeave) return { card: "border-status-warning-200 bg-status-warning-50/70", badge: "bg-status-warning-100 text-status-warning-700", dot: "bg-status-warning-500 text-text-white" };
  return { card: "border-status-success-100 bg-background-primary-default", badge: "bg-status-success-50 text-status-success-700", dot: "bg-status-success-500 text-text-white" };
}
...
export function AttendanceWorkspace({ students, records, tasks = [], onChange, onRequestTask, onActivity, onOpenTask, initialTarget, onInitialTargetConsumed }: { students: AppStudent[]; records: AttendanceRecord[]; tasks?: FollowupTask[]; onChange: (records: AttendanceRecord[]) => void; onRequestTask: (draft: FollowupTaskDraft) => void; onActivity?: (event: ActivityEvent) => void | (() => void); onOpenTask?: (taskId: string) => void; initialTarget?: TimelineTarget; onInitialTargetConsumed?: () => void }) {
  ...
  useInitialTargetEffect(initialTarget ? `${initialTarget.entityId || ""}|${initialTarget.studentId || ""}|${initialTarget.date || ""}` : undefined, () => {
    ...
    if (student) {
      ...
      window.setTimeout(() => document.querySelector(`[data-attendance-student-id="${CSS.escape(student.id)}"]`)?.scrollIntoView({ behavior: "smooth", block: "center" }), 80);
    }
  ...
  function commit(next: AttendanceRecord[]) { setUndo(records); undoActivityRef.current = null; onChange(next); }
  ...
  function markStudent(student: AppStudent) {
    ...
    if (!already) {
      ...
      else if (quickStatus === "leave" || quickStatus === "absent") patchStudent(student.id, normalizeAttendancePatch(current, quickStatus));
      else patchStudent(student.id, { [quickStatus]: true });
      const undoActivity = onActivity?.(createActivityEvent({ action: "status_changed", ref: { domain: "attendance", entityId: current?.id || `${date}:${student.id}`, studentId: student.id, date }, studentIds: [student.id], title: `登记出勤：${student.name}`, detail: QUICK_STATUS_OPTIONS.find(option => option.value === quickStatus)?.label || "正常" }));
      undoActivityRef.current = typeof undoActivity === "function" ? undoActivity : null;
    }
  ...
  return <div className="h-full overflow-y-auto bg-background-secondary-default p-4"><div className="mx-auto max-w-6xl space-y-4">
    <Card className="surface-enter" bodyClassName="grid gap-3 p-4 sm:grid-cols-3">{[{ k:"normal",l:"正常",c:"text-status-success-600"},{k:"leave",l:"请假",c:"text-status-warning-600"},{k:"absent",l:"缺勤",c:"text-status-danger-500"}].map(item => <div key={item.k} className="rounded-[var(--app-radius-sm)] bg-[var(--app-surface-muted)] px-4 py-3"><div className="text-caption-1-regular text-[var(--app-text-muted)]">{item.l}</div><div className={`mt-1 text-title-2-semibold ${item.c}`}>{counts[item.k as keyof typeof counts]}</div></div>)}</Card>
    <Card title="每日出勤" action={
      ...
      <div className="mb-4 flex flex-wrap items-center gap-2">
        ...
        <div className="relative ml-auto min-w-48 flex-1 sm:max-w-64"><Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-text-tertiary"/><input value={search} onChange={event => setSearch(event.target.value)} placeholder="搜索学生" className="h-9 w-full rounded-[var(--app-radius-sm)] border border-[var(--app-border)] bg-background-primary-default pl-9 pr-3 text-body-regular outline-none focus:border-accent-300 focus:ring-2 focus:ring-accent-500/10"/></div>
      ...
      {viewMode === "quick" ? <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">{rows.map(student => {
        ...
        return <button key={student.id} type="button" data-attendance-student-id={student.id} aria-label={`${student.name}当前${summary}，点击设为${targetLabel}`} onClick={() => markStudent(student)} className={`flex min-h-16 items-center gap-3 rounded-[var(--app-radius-sm)] border px-3 py-2 text-left transition-[border-color,background-color,box-shadow] duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-500/25 motion-reduce:transition-none ${style.card} ${recentUpdate?.studentId === student.id || focusedStudentId === student.id ? "ring-2 ring-accent-400 ring-offset-1 shadow-md" : ""}`}>
