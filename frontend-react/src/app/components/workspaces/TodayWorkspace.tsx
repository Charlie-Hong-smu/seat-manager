import { BookOpenCheck, CalendarDays, CheckCircle2, ClipboardList, FileSpreadsheet, LayoutGrid, Sparkles, UserRoundCheck } from "lucide-react";
...
import type { AppStudent, AttendanceRecord, ClassScheduleV1, CommunicationDraft, FollowupTask, HomeworkAssignment } from "../../state/types";
import { AiGenerationPanel, Button, Card, FileDropZone, ToolDrawer } from "../ui";

export function TodayWorkspace({ students, attendance, tasks, homework, schedule, drafts, onScheduleChange, onDraftsChange, onOpenSeats, onOpenAttendance, onOpenTasks, onOpenHomework, onOpenQuickRecord }: {
  students: AppStudent[];
  ...
  const dueHomework = homework.filter(item => item.dueDate <= today).length;
  const range = getWeekRange();
  const facts = buildWeeklyFacts({ students, attendance, tasks, homework, ...range });

  function openWeekly() {
    setWeeklyContent(buildLocalWeeklyDraft("班级", range.startDate, range.endDate, facts));
    setWeeklyGeneratedBy("local");
  ...
  function saveWeekly() {
    const draft: CommunicationDraft = { id: `communication-${Date.now()}`, scope: "class", startDate: range.startDate, endDate: range.endDate, facts, content: weeklyContent.trim(), generatedBy: weeklyGeneratedBy, sourceDigest: digestFacts(facts), updatedAt: new Date().toISOString() };
