  ...
  return <>
    <ToolDrawer open={open} title="快捷记录" onClose={onClose}>
      <div className="space-y-4">
        ...
        <textarea rows={4} value={note} onChange={event => setNote(event.target.value)} placeholder="记录客观事实" className="w-full resize-none rounded-[var(--app-radius-sm)] border border-[var(--app-border)] px-3 py-2 text-body-regular outline-none focus:border-accent-300" />
        <label className="block"><span className="mb-1.5 block text-caption-1-semibold text-text-tertiary">可选分值</span><input type="number" value={score} onChange={event => setScore(event.target.value)} placeholder="不填则只记录" className="h-10 w-full rounded-[var(--app-radius-sm)] border border-[var(--app-border)] px-3 text-body-regular outline-none focus:border-accent-300" /></label>
        ...
        {managing && <div className="space-y-3 rounded-[var(--app-radius-md)] border border-[var(--app-border)] bg-background-secondary-default p-3">
          <div className="flex gap-2"><input value={newPresetLabel} onChange={event => setNewPresetLabel(event.target.value)} placeholder="新预设名称" className="h-9 min-w-0 flex-1 rounded-xl border border-border-button-default bg-background-primary-default px-3 text-body-regular"/><Button size="sm" disabled={!newPresetLabel.trim()} onClick={addPreset}><Plus className="h-4 w-4"/>保存当前内容</Button></div>
