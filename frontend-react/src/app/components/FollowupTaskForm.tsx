...
export function FollowupTaskForm({ students, value, onChange, onSubmit, onCancel, showSource = true, showPlannedDate = true, compact = false, taskTypes, onTaskTypesChange }: FollowupTypeCatalogProps & {
  ...
  return <div className="space-y-3">
    ...
    <div className="flex gap-2 pt-1">{onCancel && <Button variant="ghost" onClick={onCancel} className="flex-1"><X className="h-4 w-4"/>取消</Button>}<Button disabled={!value.title.trim() || (individual && !selectedIds.length)} onClick={onSubmit} className="flex-1"><Plus className="h-4 w-4"/>{value.id ? "保存修改" : value.sourceRef?.domain === "homework" && selectedIds.length > 1 ? `创建作业跟进 · ${selectedIds.length} 人` : individual && selectedIds.length > 1 ? `分别创建 ${selectedIds.length} 项` : "创建任务"}</Button></div>
  </div>;
}
