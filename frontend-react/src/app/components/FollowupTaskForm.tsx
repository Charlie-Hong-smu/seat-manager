...
export function FollowupTaskForm({ students, value, onChange, onSubmit, onCancel, showSource = true, showPlannedDate = true, compact = false }: {
  ...
  return <div className="space-y-3">
    ...
    <label className="block"><span className="mb-1.5 block text-caption-1-semibold text-[var(--app-text-muted)]">标题</span><input value={value.title} onChange={event => onChange({ ...value, title: event.target.value })} placeholder="跟进事项，例如：确认处罚执行情况" className="h-10 w-full rounded-[var(--app-radius-sm)] border border-[var(--app-border)] bg-background-primary-default px-3 text-body-regular outline-none focus:border-accent-300 focus:ring-2 focus:ring-accent-500/10"/></label>
    <label className="block"><span className="mb-1.5 block text-caption-1-semibold text-[var(--app-text-muted)]">说明</span><textarea rows={compact ? 3 : 4} value={value.description} onChange={event => onChange({ ...value, description: event.target.value })} placeholder="补充说明（可选）" className="w-full resize-none rounded-[var(--app-radius-sm)] border border-[var(--app-border)] bg-background-primary-default px-3 py-2 text-body-regular outline-none focus:border-accent-300 focus:ring-2 focus:ring-accent-500/10"/></label>
