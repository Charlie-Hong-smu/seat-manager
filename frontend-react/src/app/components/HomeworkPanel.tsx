...
const STATUS_META: Record<HomeworkStudentStatus, { label: string; card: string; badge: string }> = {
  unrecorded: { label: "待登记", card: "border-border-button-default bg-background-primary-default hover:border-accent-200 hover:bg-accent-50/40", badge: "bg-background-tertiary-default text-text-secondary" },
  submitted: { label: "已交", card: "border-status-success-100 bg-status-success-50/55 hover:border-status-success-200", badge: "bg-status-success-100 text-status-success-700" },
  pending: { label: "未交", card: "border-status-rose-100 bg-status-rose-50/55 hover:border-status-rose-200", badge: "bg-status-rose-100 text-status-rose-700" },
  resubmitted: { label: "补交", card: "border-accent-100 bg-accent-50/55 hover:border-accent-200", badge: "bg-accent-100 text-accent-700" },
  excused: { label: "免交", card: "border-status-warning-100 bg-status-warning-50/55 hover:border-status-warning-200", badge: "bg-status-warning-100 text-status-warning-700" },
};
...
export function HomeworkPanel({ students, assignments, tasks, subjectCatalog, onChange, onTaskChange, onOpenTask, onSubjectCatalogChange, onCreateFollowups, onActivity, initialAssignmentId }: {
  ...
  return <>
    <div className="grid gap-4 lg:grid-cols-[20rem_minmax(0,1fr)]">
      <div className="space-y-4">
        <Card title="布置作业"><div className="space-y-3">
          <input value={title} onChange={event => setTitle(event.target.value)} placeholder="作业名称" className="h-10 w-full rounded-[var(--app-radius-sm)] border border-[var(--app-border)] px-3 text-body-regular outline-none focus:border-accent-300 focus:ring-2 focus:ring-accent-500/10"/>
          ...
          <textarea value={note} onChange={event => setNote(event.target.value)} rows={3} placeholder="说明（可选）" className="w-full resize-none rounded-[var(--app-radius-sm)] border border-[var(--app-border)] px-3 py-2 text-body-regular outline-none focus:border-accent-300 focus:ring-2 focus:ring-accent-500/10"/>
        ...
        {selected ? <div className="space-y-4">
          <div className="rounded-[var(--app-radius-md)] bg-[var(--app-surface-muted)] p-3">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <div className="min-w-36 text-body-semibold text-[var(--app-text)]">登记进度 <span className="ml-1 text-title-3-regular font-black">{registeredCount} / {participantStudents.length}</span></div>
    ...
    <ToolDrawer open={manageOpen} title="管理作业" onClose={() => setManageOpen(false)} footer={<div className="flex gap-2"><Button variant="ghost" className="flex-1" onClick={() => setManageOpen(false)}>取消</Button><Button className="flex-1" disabled={!editTitle.trim() || !editSubject} onClick={saveAssignment}>保存修改</Button></div>}>
      <div className="space-y-4">
        <input value={editTitle} onChange={event => setEditTitle(event.target.value)} placeholder="作业名称" className="h-10 w-full rounded-[var(--app-radius-sm)] border border-[var(--app-border)] px-3 text-body-regular outline-none focus:border-accent-300"/>
        ...
        <textarea value={editNote} onChange={event => setEditNote(event.target.value)} rows={4} placeholder="作业说明" className="w-full resize-none rounded-[var(--app-radius-sm)] border border-[var(--app-border)] px-3 py-2 text-body-regular outline-none focus:border-accent-300"/>
        {selected && <div className="rounded-[var(--app-radius-md)] border border-[var(--app-border)] bg-[var(--app-surface-muted)] p-3 text-caption-1-regular leading-5 text-[var(--app-text-muted)]">参与学生 {participantStudents.length} 人 · 关联任务 {tasks.filter(task => task.sourceRef?.domain === "homework" && task.sourceRef.entityId === selected.id).length} 项</div>}
    ...
    <ToolDrawer open={catalogOpen} title="管理常用学科" onClose={() => setCatalogOpen(false)} footer={<div className="flex gap-2"><Button variant="ghost" className="flex-1" onClick={() => setCatalogOpen(false)}>取消</Button><Button className="flex-1" onClick={saveCatalog}>保存学科设置</Button></div>}>
      <div className="space-y-4"><p className="text-body-regular leading-6 text-[var(--app-text-muted)]">这些学科属于当前班级和学期。修改名称不会重写历史作业；历史中已使用的学科会继续保留。</p><div className="space-y-2">{draftSubjects.map((item, index) => <label key={index} className="block text-caption-1-semibold text-[var(--app-text-muted)]">学科 {index + 1}<input value={item} onChange={event => setDraftSubjects(current => current.map((subjectItem, subjectIndex) => subjectIndex === index ? event.target.value : subjectItem))} aria-label={`学科 ${index + 1}`} className="mt-1 h-10 w-full rounded-[var(--app-radius-sm)] border border-[var(--app-border)] px-3 text-body-regular text-[var(--app-text)] outline-none focus:border-accent-300 focus:ring-2 focus:ring-accent-500/10"/></label>)}</div><div className="rounded-[var(--app-radius-md)] border border-[var(--app-border)] bg-[var(--app-surface-muted)] p-3"><div className="text-caption-1-semibold text-[var(--app-text-muted)]">添加学科</div><div className="mt-2 flex gap-2"><input value={newSubject} onChange={event => setNewSubject(event.target.value)} onKeyDown={event => { if (event.key === "Enter") addSubject(); }} placeholder="例如：信息技术" className="h-10 min-w-0 flex-1 rounded-[var(--app-radius-sm)] border border-[var(--app-border)] bg-background-primary-default px-3 text-body-regular outline-none focus:border-accent-300"/><Button size="sm" disabled={!newSubject.trim()} onClick={addSubject}><Plus className="h-4 w-4"/>添加</Button></div>{catalogStatus && <p role="alert" className="mt-2 text-caption-1-semibold text-status-rose-600">{catalogStatus}</p>}</div></div>
    </ToolDrawer>
    {appDialog.dialog}
    {actionToast.toast}
  </>;
