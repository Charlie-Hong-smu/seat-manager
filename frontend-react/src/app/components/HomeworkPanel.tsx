...
import { Button, Card, DatePicker, SegmentedControl, SelectMenu, ToolDrawer, useActionToast, useAppDialog, Input, Textarea } from "./ui";
...
const STATUS_META: Record<HomeworkStudentStatus, { label: string; badge: string }> = {
  unrecorded: { label: "待登记", badge: "bg-background-tertiary-default text-text-secondary" },
  submitted: { label: "已交", badge: "bg-status-success-100 text-status-success-700" },
  pending: { label: "未交", badge: "bg-status-rose-100 text-status-rose-700" },
  resubmitted: { label: "补交", badge: "bg-accent-100 text-accent-700" },
  excused: { label: "免交", badge: "bg-status-warning-100 text-status-warning-700" },
};
  ...
  return <>
    ...
    <ToolDrawer open={manageOpen} title="管理作业" onClose={() => setManageOpen(false)} footer={<div className="flex gap-2"><Button variant="ghost" className="flex-1" onClick={() => setManageOpen(false)}>取消</Button><Button className="flex-1" disabled={!editTitle.trim() || !editSubject} onClick={saveAssignment}>保存修改</Button></div>}>
      <div className="space-y-4">
        ...
        <Textarea value={editNote} onChange={setEditNote} rows={4} placeholder="作业说明" resize="none" />
    ...
    <ToolDrawer open={catalogOpen} title="管理常用学科" onClose={() => setCatalogOpen(false)} footer={<div className="flex gap-2"><Button variant="ghost" className="flex-1" onClick={() => setCatalogOpen(false)}>取消</Button><Button className="flex-1" onClick={saveCatalog}>保存学科设置</Button></div>}>
      <div className="space-y-4">
        <p className="text-body-regular leading-6 text-text-tertiary">这些学科属于当前班级和学期。修改名称不会重写历史作业；历史中已使用的学科会继续保留。</p>
        <div className="space-y-3">{draftSubjects.map((item, index) => <Input key={index} label={`学科 ${index + 1}`} value={item} onChange={value => setDraftSubjects(current => current.map((subjectItem, subjectIndex) => subjectIndex === index ? value : subjectItem))} />)}</div>
        <div className="space-y-2"><div className="text-caption-1-semibold text-text-secondary">添加学科</div><div className="flex items-center gap-2"><Input value={newSubject} onChange={setNewSubject} onKeyDown={event => { if (event.key === "Enter") addSubject(); }} placeholder="例如：信息技术" className="min-w-0 flex-1" /><Button size="sm" disabled={!newSubject.trim()} onClick={addSubject}><Plus className="h-4 w-4"/>添加</Button></div>{catalogStatus && <p role="alert" className="text-caption-1-semibold text-status-rose-600">{catalogStatus}</p>}</div>
      </div>
    </ToolDrawer>
    {appDialog.dialog}
    {actionToast.toast}
