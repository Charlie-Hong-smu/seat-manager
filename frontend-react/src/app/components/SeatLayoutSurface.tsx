...
export function SeatLayoutSurface({ layout: source, detail = false, renderSeat }: {
  ...
  if (!source.slotGridVersion && !["default-grid", "grid"].includes(source.template)) {
    return <div className="relative mx-auto" data-seat-layout-surface style={{ minWidth: getSeatLayoutStageMinWidth(source), aspectRatio: `${source.canvas.width}/${source.canvas.height}` }}>
      {source.groups.filter(group => group.shape === "round").map(group => {
        ...
        return <div key={group.id} className="pointer-events-none absolute grid h-20 w-28 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-[50%] border border-status-warning-200 bg-status-warning-50 text-caption-1-regular text-status-warning-700" style={{ left: `${x / source.canvas.width * 100}%`, top: `${y / source.canvas.height * 100}%` }}>{group.name}</div>;
      })}
  ...
  return <div className="seat-layout-surface" data-seat-layout-surface data-detail={detail ? "true" : "false"} style={{ minWidth: `${columns * 76}px` }}>
    ...
    <div className="seat-layout-surface__grid" style={{ gridTemplateColumns: `repeat(${columns}, minmax(0, 1fr))`, gridTemplateRows: `repeat(${rows}, minmax(${detail ? 78 : 32}px, 1fr))`, minHeight: rows * (detail ? 78 : 32), maxHeight: rows * (detail ? 94 : 58) }} onPointerLeave={() => setHovered(null)}>
