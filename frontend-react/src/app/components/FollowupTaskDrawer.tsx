...
import { Button, ToolDrawer } from "./ui";
...
export function FollowupTaskDrawer({ open, students, draft, onClose, onConfirm }: { open: boolean; students: AppStudent[]; draft: FollowupTaskDraft | null; onClose: () => void; onConfirm: (draft: FollowupTaskDraft) => void }) {
  ...
  return <ToolDrawer open={open} title={value.id ? "编辑跟进任务" : "创建跟进任务"} onClose={onClose}>
    <div className="space-y-4"><div className="rounded-xl border border-violet-100 bg-violet-50 px-3 py-2 text-xs leading-5 text-violet-700">来源：{value.source === "dormitory" ? "宿舍处理" : value.source === "attendance" ? "出勤异常" : value.source === "ai" ? "AI 建议" : "手动创建"}。这是其他功能预填的任务，请确认内容后创建。</div>
      ...
      <div className="grid grid-cols-2 gap-3"><label><span className="mb-1.5 block text-xs font-bold text-gray-400">计划日期</span><div className="flex items-center rounded-xl border border-gray-200 px-3"><CalendarClock className="h-4 w-4 text-gray-400"/><input type="date" value={value.plannedDate} onChange={event => setValue({ ...value, plannedDate: event.target.value })} className="h-10 min-w-0 flex-1 bg-transparent pl-2 text-sm outline-none"/></div></label><label><span className="mb-1.5 block text-xs font-bold text-gray-400">截止日期</span><input type="date" value={value.dueDate} onChange={event => setValue({ ...value, dueDate: event.target.value })} className="h-10 w-full rounded-xl border border-gray-200 px-3 text-sm outline-none"/></label></div>
