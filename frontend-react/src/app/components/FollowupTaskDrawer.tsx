...
export function FollowupTaskDrawer({ open, students, draft, onClose, onConfirm }: { open: boolean; students: AppStudent[]; draft: FollowupTaskDraft | null; onClose: () => void; onConfirm: (draft: FollowupTaskDraft) => void }) {
  ...
  return <ToolDrawer open={open} title={value.id ? "编辑跟进任务" : "创建跟进任务"} onClose={onClose}>
    <div className="space-y-4"><div className="rounded-xl border border-violet-100 bg-violet-50 px-3 py-2 text-xs leading-5 text-violet-700">来源：{value.source === "dormitory" ? "宿舍处理" : value.source === "attendance" ? "出勤异常" : value.source === "ai" ? "AI 建议" : "手动创建"}。这是其他功能预填的任务，请确认内容后创建。</div>
      <StudentMultiPicker students={students} values={value.studentIds?.length ? value.studentIds : value.studentId ? [value.studentId] : []} onChange={studentIds => setValue(current => current ? { ...current, studentIds, studentId: studentIds[0] || "" } : current)} label="关联学生（可选）" emptyLabel="不指定学生" />
      ...
      <div className="flex gap-2 pt-2"><Button variant="ghost" onClick={onClose} className="flex-1"><X className="h-4 w-4"/>取消</Button><Button disabled={!value.title.trim()} onClick={() => onConfirm({ ...value, title: value.title.trim(), description: value.description.trim() })} className="flex-1">{value.id ? "保存修改" : "确认创建"}</Button></div>
    </div>
  </ToolDrawer>;
}
