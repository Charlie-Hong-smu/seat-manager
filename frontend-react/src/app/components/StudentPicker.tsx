...
export function StudentPicker({ students, value, onChange, label = "选择学生", allowClear = false, compact = false, className = "" }: {
  ...
  return <>
    ...
    {createPortal(<AnimatedPopover open={open} className="fixed z-[120] overflow-hidden rounded-[var(--app-radius-md)] border border-[var(--app-border)] bg-background-primary-default p-2 shadow-[var(--app-shadow-float)]" style={{ left: position.left, top: position.top, width: position.width, maxHeight: position.maxHeight }}>
      <div ref={panelRef}>
        <div className="relative mb-2"><Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-text-tertiary"/><input autoFocus value={search} onChange={event => setSearch(event.target.value)} placeholder="搜索姓名或别名" className="h-9 w-full rounded-[var(--app-radius-sm)] border border-[var(--app-border)] bg-[var(--app-surface-muted)] pl-9 pr-3 text-body-regular outline-none focus:border-accent-300 focus:bg-background-primary-default"/></div>
        <div role="listbox" aria-label={label} className="max-h-64 space-y-1 overflow-y-auto">{allowClear && <button type="button" role="option" aria-selected={!value} onClick={() => { onChange(""); setOpen(false); setSearch(""); triggerRef.current?.focus(); }} className="flex h-10 w-full items-center rounded-[var(--app-radius-sm)] px-3 text-body-regular text-text-tertiary hover:bg-[var(--app-surface-muted)]">不指定学生</button>}{candidates.map(student => <button key={student.id} type="button" role="option" aria-selected={student.id === value} onClick={() => { onChange(student.id); setOpen(false); setSearch(""); triggerRef.current?.focus(); }} className={`flex h-10 w-full items-center rounded-[var(--app-radius-sm)] px-3 text-body-regular transition-colors ${student.id === value ? "bg-accent-50 font-bold text-accent-700" : "text-text-primary hover:bg-[var(--app-surface-muted)]"}`}><span className="flex-1 text-left">{student.name}</span>{student.id === value && <Check className="h-4 w-4"/>}</button>)}{!candidates.length && !allowClear && <div className="py-8 text-center text-body-regular text-[var(--app-text-muted)]">没有匹配学生</div>}</div>
      </div>
...
export function StudentMultiPicker({ students, values, onChange, label = "选择学生", emptyLabel = "请选择学生" }: { students: AppStudent[]; values: StudentId[]; onChange: (ids: StudentId[]) => void; label?: string; emptyLabel?: string }) {
  ...
  return <div className="relative">
    ...
    <AnimatedPopover open={open} className="absolute inset-x-0 top-full z-30 mt-2 overflow-hidden rounded-[var(--app-radius-md)] border border-[var(--app-border)] bg-background-primary-default p-2 shadow-[var(--app-shadow-float)]">
      <div className="relative mb-2"><Search className="pointer-events-none absolute left-3 top-2.5 h-4 w-4 text-text-tertiary"/><input value={search} onChange={event => setSearch(event.target.value)} placeholder="搜索姓名或别名" className="h-9 w-full rounded-[var(--app-radius-sm)] border border-[var(--app-border)] bg-[var(--app-surface-muted)] pl-9 pr-3 text-body-regular outline-none transition-colors focus:border-accent-300 focus:bg-background-primary-default focus:ring-2 focus:ring-accent-500/10"/></div>
      <div ref={candidatesContainerRef} className="max-h-64 space-y-1 overflow-y-auto">{candidates.map(student => <button key={student.id} data-selection-motion-id={student.id} type="button" onClick={event => toggleWithAnimation(event, student)} className={`flex h-10 w-full items-center rounded-[var(--app-radius-sm)] px-3 text-body-regular transition-colors ${selectedIds.has(student.id) ? "bg-accent-50 font-bold text-accent-700" : "text-text-primary hover:bg-[var(--app-surface-muted)]"}`}><span className="flex-1 text-left">{student.name}</span>{selectedIds.has(student.id) ? <Check className="h-4 w-4"/> : <span className="text-[10px] font-semibold text-accent-400">加入</span>}</button>)}{!candidates.length && <div className="py-8 text-center text-body-regular text-[var(--app-text-muted)]">没有匹配学生</div>}</div>
    </AnimatedPopover>
  </div>;
}
