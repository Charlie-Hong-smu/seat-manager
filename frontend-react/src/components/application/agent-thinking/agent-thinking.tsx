...
function DotsIndicator({ variant }: { variant: "wave" | "spin" }) {
  ...
  useEffect(() => {
    ...
    const start = () => {
      ...
      if (media.matches) { setOpacities(DOTS_SEED); return; }
      let phase = 0;
    ...
    return () => { window.clearInterval(timer); media.removeEventListener("change", start); };
  }, [variant]);

  return (
    <span
      aria-hidden
      data-agent-thinking-indicator={variant}
