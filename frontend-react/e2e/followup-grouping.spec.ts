...
test("collection followups retain separate students, statuses and history records", async ({ page }) => {
  ...
  await page.getByRole("button", { name: /^任务与作业/ }).click();
  const group = page.locator("article").filter({ hasText: "班费收缴：秋季班费" });
  await expect(group).toHaveCount(1);
  const cards = page.locator("[data-followup-task-id]");
  await expect(cards).toHaveCount(3);
  const first = cards.filter({ hasText: "关联甲" });
  await first.getByRole("button", { name: "编辑任务：关联甲" }).click();
  ...
  await first.getByRole("button", { name: "完成任务：关联甲" }).click();
  await expect(cards.filter({ hasText: "关联乙" }).getByRole("button", { name: "完成任务：关联乙" })).toBeVisible();
  await expect.poll(async () => (await storedTasks(page)).filter((task: { status: string }) => task.status === "completed").length).toBe(1);
  await group.getByRole("button", { name: "全部完成" }).click();
  await expect.poll(async () => (await storedTasks(page)).filter((task: { status: string }) => task.status === "completed").length).toBe(3);
  await page.getByRole("status").filter({ hasText: "已完成 2 项跟进" }).getByRole("button", { name: "撤销" }).click();
  await expect.poll(async () => (await storedTasks(page)).filter((task: { status: string }) => task.status === "completed").length).toBe(1);
  await page.getByRole("navigation", { name: "主导航" }).getByRole("button", { name: "今日", exact: true }).click();
  await page.getByRole("button", { name: "完成全部跟进：班费收缴：秋季班费" }).click();
  await expect.poll(async () => (await storedTasks(page)).filter((task: { status: string }) => task.status === "completed").length).toBe(3);
  await page.getByRole("status").filter({ hasText: "已完成 2 项跟进" }).getByRole("button", { name: "撤销" }).click();
  await expect.poll(async () => (await storedTasks(page)).filter((task: { status: string }) => task.status === "completed").length).toBe(1);
